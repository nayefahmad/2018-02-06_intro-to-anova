...
year2017 <- data.frame(date=seq(ymd("2017-10-01"), 
                                ymd("2018-02-28"), 
                                by="day")) %>% 
...
# summarized version: 
predict2017.no.trend.summary <- 
      group_by(predict2017.no.trend,
               day.of.week,
               month) %>% 
      summarize(average=mean(num.visits)) %>% 
      as.data.frame %>% print
...
write_csv(visits,
          here("results", 
               "output from src", 
               "2017-10-24_vgh-ed-visits-formatted.csv"))
